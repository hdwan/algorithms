...
**定义**：$a,b\in Z,a\ne0$，若 $b$ 除以 $a$，**商为整数**，且**余数为零**，即 $a|b$（“|”是整除符号），读作“$a$ 整除 $b$”或“$b$ 能被 $a$ 整除”。
...
1. ##### 概念
    ...
    每一个数字都可以表示为k个质数(质因子)的 a~i~ 次方的成绩。
    ...
    if (a > 1) //对于任意数a，大于根号a的质因子最多有一个，否则质因子平方会大于a，矛盾了。
       hashmap[a] ++;
    for (auto it = hashmap.begin(); it != hashmap.end(); it ++) {
       // 针对一个很大的质数  那么会计算 本身*(本身 - 1) / 本身 不使用long long 会爆炸
...
1. ##### 线性筛法求欧拉函数
    ...
    对于非质数：
    如果 $i \mod prime[j] == 0$，即 $prime[j]$ 是 $i$ 的质因数，因为欧拉函数包含两部分：原数字 $n$ 和后面的分式，所以计算 $prime[j] * i$ 的欧拉函数后面得分式就等于计算 $i$ 得欧拉函数得分式，（$prime[j]$ 是 $prime[j] * i$ 的质因数），变得是前面的原数字，即再乘上一个 $prime[j]$。
    如果 $i \mod prime[j] != 0$，即 $prime[j]$ 不是 $i$ 的质因数，则 $prime[j]\*i$ 的欧拉函数等于 $prime[j]$ 的乘上 $i$的，即 $(prime[j]- 1)*phi[i]$。

1. **欧拉定理**

    如果 $\gcd(a,m)=1$，则$a^{\varphi(m)}\equiv1(\mod m)$。
...
1. **裴蜀定理**
    ...
    对于任意的整数对$a$，$b$，且 $\gcd(a,b)=d$，那么对于任意的整数 $x$，$y$，$ax+by$ 都一定是 $d$ 的倍数，特别地，**一定存在**整数 $x$，$y$，使 $ax+by=d$ 成立。

    **推论**：a，b**互质的充分必要条件**是：存在整数 $x$，$y$ 使 $ax+by=1$。即二者的最大公约数是 1，所以互质。

1. **扩展欧几里得**

    对于裴蜀定理的特例：$ax+by=\gcd(a, b)$ ，求 $x、y$。
...
如果一个线性同余方程 $ax\equiv 1(\mod b)$，则 $x$ 称为 $a\mod b$ 的**逆元**，记作 $a^{-1}$。

**判断条件**： 存在乘法逆元的**充要条件**是 **$b$ 与模数 $m$ 互质**。当**模数 `m` 为质数**时， 即为 $b$ 的乘法逆元。

**费马小定理**：假如 $a$ 是**整数**， $p$ 是**质数**（质数），那么 $a^p-a$ 是 $p$ 的倍数（即 $a^p$ 对 $p$ 取模余数为 $a$），可以表示为 $a^p\equiv a(\mod p)$。
...
![image-20230817193615425](typora文档图片/image-20230817193615425.png)
...
2. 对于第 i 个方程：
    - 计算 $m_i=\frac{n}{n_i}$；
    - 计算 $m_i$ 在模 $n_i$ 意义下的逆元 $m_i^{-1}$![m_i^{-1}](data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7)；
