...
3. **最大公约数**
    ...
    2. **欧几里得算法**
        ...
        **时间复杂度**：O(log(max(a, b)))，计算过程：
        ...
        当 $a\operatorname <b$ 的时候，$\gcd(a,b)=\gcd(b,a%b)$，$a'=b,b'=a%b\in(0,b-1)$，此时的$a'\operatorname{>}b'$。

        而对于 $a>b$ 的情况：

        - 当 $a>2b$ 时：$\gcd(a,b)=\gcd(b,a\%b)$，$a'=b,b'=a%b$，$b'\in(0,b-1)<\frac{a}{2}$，即 $a$ 的大小至少减半，所以时间复杂度为 $O(\log(n))$；
        - 当 $a \operatorname = {2b}$ 时：$\gcd(a,b)=\max(2,b)$，时间复杂度为 $O(1)$。
        - 当 $a\operatorname <{2b}$ 时：$\gcd(a,b)=\gcd(b,a%b)$，$a'=b,b'=a%b\in(0,b-1)$，此时 $\gcd(a',b')$ 又变成 $a>b$，即最坏情况下时间复杂度为 $O(2\log(n))$；
